...
**Example:** Rendered Response Manipulation

You can use plugins to register a hook that can manipulate the renderer response.

```ts
export default defineNitroPlugin((nitro) => {
  nitro.hooks.hook('render:response', (response) => {
    console.log(`Response status is: ${response.statusCode}`)
    manipulateResponse(response) // Manipulate the renderer response here
  })
})
```
